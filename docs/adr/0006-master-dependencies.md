...
- New global config shape (a flat list of `{master: {path, type}, depends_on: {path, type}}` edges) and a new per-rule `ignore_dependencies`-style flag, both still to be worked out at the field/TOML-shape level — tracked in [Master dependencies: decision map](https://github.com/LydP/syncer/issues/52).
